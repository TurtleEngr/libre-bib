...
- The DB packages, mariadb-client and mariadb-server, have been
  installed. Most likely the mariadbd process will already
...
If that doesn't work look at the cgDbHost variable setting in
conf.env. It should be set to "localhost" or "127.0.0.1" for local
...
+ Var: **cgDirCache="$cgDirTmp"**
  ...
  Sensitive files, such as passwords, will be put in this
  directory. For good security this directory could be on a mounted
  USB thumb drive, or an encrypted disk. If this is not found,
  programs that need DB access will not continue.
...
<p>If you run import-lo with updated entries, then run
